	...
	motors_en(true)
{
	...
	motors_freewheel_sub = nh.subscribe(
		"motors_freewheel",
		1,
		&RobotHardwareInterface::motors_freewheel_cb,
		this
	);
...
//-----------------------------------
void RobotHardwareInterface::motors_freewheel_cb(
	const std_msgs::Bool::ConstPtr& msg
) {
	motors_freewheel = msg->data;
	ROS_INFO(
		"Motors freewheel %s", motors_freewheel ? "enabled" : "disabled"
	);
}
